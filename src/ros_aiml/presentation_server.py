...
def make_wavs(loc_name, level):
	...
	if len(files) == 0:
		rospy.loginfo("Location or level not found (location: %s, level: %s)"%(loc_name, level))
		return
	if len(files) == 4:
	...
	wholetext.replace("\n", " ")
	xmlelems = re.compile(r'<[^>]>')
	noxml = xmlelems.sub('', wholetext) #strip out those xml elements that Watson can interpret
	splittext = re.split(r'[{}]', noxml)
	...
	#This means splittext[0] will always be a speech section (possibly empty, though)
	speeches = []
	gestures = []
	...
	timlist = []
	j=0
	#there's a fundamental problem here trying to integrate the watson expression tags: how will it behave with mismatched tags?
	#Because that's what will happen when the text is split by gesture
		#Malformed tags cause Watson to return an empty file. Guess we need to ignore them, for now.
	for i in range(len(gestures)):
		if speeches[i] is "":
			timlist.append((gestures[i], 0.0))
		else:
...
def find_pres_files(loc_name, level):
	...
	try:
		...
		return () #this can also evaluate to false, right?
	else:
		f0.close()
	try:
...
def run_pres(loc_name, level):
	gsender = GestureSender()#and if this can't be made, the service freezes. e.g. if run without Gazebo open
	ssender = SoundClient()
	rospy.sleep(1)
	if(loc_name == "secret"):
		#I should probably scrap this in favour of Bijin's play_sound thingy
	...
	if len(files) == 4:
		...
		#running straight from script! Copy make_wavs process (TODO: extract into another function)
		with open(files[1].name, 'r') as script:
			...
			for elem in splittext:
				if doinggest == True:
					if gsender.running():
						gsender.quitIt() #This is what causes choppy delivery, the necessary sleep() after cancelling goals before sending a new one (which is built into that class)
					#How to get around?
					gsender.send_motion_free(elem)
				else:
					##old code
					ssender.say(elem)
					rospy.sleep(0.5) #have to sleep for at least half a second to guarantee the diagnostics channel has registered the playing sound
					while _sound_running:
						print("Speaking, and sound is running, yeah?")
						rospy.sleep(0.2)
						pass
